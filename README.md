...
Most performance mods make the game faster and stop there. Graphene starts from a different
...
make each frame cheaper, Graphene decides how much work to put in a frame. Both are useful and they
...
Graphene reads the renderer's own visible-section set and skips entities that are not in it. It costs
...
Graphene scores each by how much of the screen it can account for and admits them against a per-frame
...
### 4. Tunes itself to your machine — `/graphene autotune`
...
So Graphene measures instead. It:
...
Graphene adds that case and makes the thresholds configurable, and always takes the *lower* of its cap
...
bug. The overlay (`/graphene hud full`) shows the frame-time trace with the target drawn across it,
...
| Command | Effect |
| --- | --- |
| `/graphene` | Current state: frame rate, 1% low, quality, what is being culled |
| `/graphene target <fps>` | Set the frame-rate target |
| `/graphene autotune [fps]` | Search for the best settings this machine can sustain |
| `/graphene mode adaptive\|fixed\|observe` | Steer continuously, use a fixed profile, or measure only |
| `/graphene hud off\|compact\|full` | Diagnostic overlay |
| `/graphene on` / `off` | Master switch |
| `/graphene cancel` | Stop a running tune |

`observe` mode is worth knowing about: it measures and reports without changing anything, which is
how you find out whether Graphene would help before letting it touch the picture.

Settings live in `config/graphene.properties` as commented, hand-editable text. Bad values fall back
...
- **`core/`** — the engine. Frame-time statistics, the control loop, the degradation ladder, the
  ...
  decision Graphene makes is made here, which is why all of it can be tested properly.
...
```bash
./gradlew build     # jar at fabric/build/libs/graphene-0.1.0.jar
...
- **JVM advice is advice.** Heap size and garbage collector are launcher settings a mod cannot change
  at runtime. Graphene logs what is set and what would be better, and does not pretend otherwise.
