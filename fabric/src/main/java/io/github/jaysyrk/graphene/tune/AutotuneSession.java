package io.github.jaysyrk.graphene.tune;

import io.github.jaysyrk.graphene.Graphene;
import io.github.jaysyrk.graphene.client.GrapheneRuntime;
import io.github.jaysyrk.graphene.core.autotune.AutotuneConfig;
import io.github.jaysyrk.graphene.core.autotune.AutotuneResult;
import io.github.jaysyrk.graphene.core.autotune.Autotuner;
import io.github.jaysyrk.graphene.core.autotune.Trial;
import io.github.jaysyrk.graphene.core.autotune.TrialResult;
...
public final class AutotuneSession {
    ...
    public AutotuneSession(GrapheneRuntime runtime) {
    ...
    public boolean start(int targetFps, Consumer<Component> reporter) {
        ...
        this.tuner.start();
        beginTrial();
        Graphene.LOGGER.info("Autotune started, target {} fps", targetFps);
        return true;
    }
    ...
    private void reportProgress() {
        ...
        reporter.accept(Component.literal(String.format(
                "Graphene: tuning %d%% - %s", Math.round(tuner.progress() * 100), next.label())));
    }
    ...
    private void finish() {
        ...
        trial = null;
        if (result == null) {
            reporter.accept(Component.literal("Graphene: tuning stopped."));
            return;
        }
        for (String line : result.log()) {
            Graphene.LOGGER.info("[autotune] {}", line);
        }
        if (!result.targetReached()) {
            reporter.accept(Component.literal(String.format(
                    "Graphene: %d fps is out of reach on this machine even at the lowest settings "
                            + "(measured %.0f fps). Try a lower target.",
                    tuner.config().targetFps(), result.achievedFps())));
            return;
        }
        reporter.accept(Component.literal(String.format(
                "Graphene: tuning done in %d trials. Holding %.0f fps at %d%% visual quality. "
                        + "Run '/graphene mode fixed' to keep it.",
    ...
    public void cancel() {
        if (tuner != null && tuner.isRunning()) {
            tuner.cancel();
            reporter.accept(Component.literal("Graphene: tuning cancelled."));
        }
        tuner = null;
        trial = null;
        frameInTrial = 0;
