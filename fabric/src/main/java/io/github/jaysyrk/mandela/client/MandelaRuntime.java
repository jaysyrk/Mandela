...
import io.github.jaysyrk.mandela.core.governor.QualityGovernor;
import io.github.jaysyrk.mandela.core.idle.IdlePolicy;
import io.github.jaysyrk.mandela.core.idle.WindowState;
...
import io.github.jaysyrk.mandela.core.visibility.RenderBudget;
import io.github.jaysyrk.mandela.core.visibility.VisibilityCache;
import io.github.jaysyrk.mandela.tune.AutotuneSession;
 ...
 */
public final class MandelaRuntime {
    ...
    private MandelaConfig config = new MandelaConfig();
    ...
    public void endFrame() {
        ...
        if (config.isEnabled() && config.getMode() == MandelaConfig.Mode.ADAPTIVE) {
    ...
    /** Whether Mandela should be changing anything right now. */
    ...
    /**
     * The frame cap Mandela wants, or {@link IdlePolicy#UNCAPPED}.
     *
    ...
    public MandelaConfig config() {
        return config;
    }

    /** Replaces the configuration and pushes the parts of it the engine caches. */
    public void applyConfig(MandelaConfig config) {
        this.config = java.util.Objects.requireNonNull(config, "config");
        governor.setConfig(config.toGovernorConfig());
        governor.setEnabled(config.isEnabled() && config.getMode() == MandelaConfig.Mode.ADAPTIVE);
        idlePolicy.setConfig(config.getIdle());
        blockEntityBudget.reset();
        entityVisibility.clear();
        Mandela.LOGGER.info("Configuration applied: mode={}, target={}fps, enabled={}",
