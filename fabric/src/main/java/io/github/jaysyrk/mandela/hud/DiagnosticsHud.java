package io.github.jaysyrk.mandela.hud;
...
/**
 * Draws what Mandela is doing and why.
 *
...
public final class DiagnosticsHud implements HudRenderCallback {
    ...
    public DiagnosticsHud(MandelaRuntime runtime) {
        this.runtime = java.util.Objects.requireNonNull(runtime, "runtime");
    }

    @Override
    public void onHudRender(GuiGraphics graphics, DeltaTracker deltaTracker) {
        MandelaConfig.HudMode mode = runtime.config().getHudMode();
        if (mode == MandelaConfig.HudMode.OFF) {
            return;
        }
        ...
        if (mode == MandelaConfig.HudMode.FULL) {
    ...
    private int colourForPhase(GovernorState governor) {
        if (runtime.config().getMode() != MandelaConfig.Mode.ADAPTIVE) {
            return COLOUR_TEXT;
        }
